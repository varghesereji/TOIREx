...
def mediancomb_sky_subtr(frames_list, opdir, config, group,
                         txtfile):
    """
    Create a median-combined sky frame and subtract it from each input frame.

    The input frames are first combined using a median combination with
    percentile-based scaling (``p50``). The resulting sky frame is then
    subtracted from each individual input frame. The names of the
    sky-subtracted frames are also written to the provided text file.

    Parameters
    ----------
    frames_list : list of str or pathlib.Path
        List of input frame filenames. The files are assumed to be
        located in `opdir`.

    opdir : str or pathlib.Path
        Directory containing the input frames and where the combined sky
        frame and sky-subtracted frames will be written.
    ...
    group : str or int
        ...
        median-combined sky frame.

    txtfile : file-like object
        Open text file used to record the names of the sky-subtracted
        frames. Existing contents of the file are cleared before writing
        the new frame names.

    Returns
    -------
    None
        The median-combined sky frame and sky-subtracted frames are
        written to `opdir`.

    Notes
    -----
    The median-combined sky frame is saved as::

        mediancomb_bkg{group}.fits

    Each input frame is sky-subtracted using this combined sky frame.
    The resulting filename is generated by replacing the input frame's
    suffix with ``.skysub.fits``.
